# CLAUDE.md — Epiphany Engine

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Epiphany Engine
- **Role**: Epiphany Engine
- **URL**: https://epiphany-engine.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/epiphany-engine
- **Branch**: master
...
### Deploy
```bash
cd /tmp/epiphany-engine && wrangler deploy
```

### Health Check
```bash
curl -s https://epiphany-engine.casey-digennaro.workers.dev/health
curl -s https://epiphany-engine.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/problem | API endpoint |
| /api/problems | API endpoint |
| /api/solution | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=epiphany-engine
...
- **vessel.json capabilities**: problem-decomposition, vessel-assignment, synthesis
...
1. Keep the vessel small. ~152 lines is the current size.
